        ...
        buildPlayer: function(options) {
            ...
            return this.each(function() {
                ...
                if (YTPlayer.opt.addRaster) {
                    ...
                    overlay.css({
                        backgroundImage: "url(" + (retina ? jQuery.mbYTPlayer.rasterImgRetina : jQuery.mbYTPlayer.rasterImg) + ")"
                    });
                }
...
*/
                    ...
                    if (!jQuery.browser.msie) { //YTPlayer.opt.ratio == "auto" &&
                        jQuery.getJSON('http://gdata.youtube.com/feeds/api/videos/' + videoID + '?v=2&alt=jsonc', function(data, status, xhr) {
                            ...
                            if (!YTPlayer.isBackground) {
                                ...
                                $YTPlayer.css({
                                    background: "url(" + bgndURL + ") center center",
        ...
        buildYTPControls: function() {
            ...
            var viewOnYT = jQuery(jQuery.mbYTPlayer.controls.ytLogo).on("click", function() {
                window.open(data.videoURL, "viewOnYT")
            });
            var viewOnlyYT = jQuery(jQuery.mbYTPlayer.controls.onlyYT).on("click",
                function() {
                    if (!YTPlayer.isAlone) {
                        jQuery(YTPlayer.wrapper).css({
                            zIndex: 10000
                        }).CSSAnimate({
                            opacity: 1
                        }, 1000, 0);
                        ...
                        jQuery(YTPlayer.wrapper).CSSAnimate({
                            opacity: YTPlayer.opt.opacity
                        }, 500, function() {
                            jQuery(YTPlayer.wrapper).css({
                                zIndex: -1
